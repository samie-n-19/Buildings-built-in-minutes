...
class NeRFDataset(Dataset):
    def __init__(self, root_dir, split="train"):
        ...
        self.frames = [
            ...
            if not frame["file_path"].endswith("_depth_0001")  # Exclude depth images
        ]
        self.camera_angle_x = self.data["camera_angle_x"]

        # Only convert image to tensor, no resizing
        self.transform = transforms.ToTensor()

        # Extract image resolution from first image
...
if __name__ == "__main__":
    dataset = NeRFDataset(root_dir=os.path.abspath("../archive/nerf_synthetic/lego"), split="test")
